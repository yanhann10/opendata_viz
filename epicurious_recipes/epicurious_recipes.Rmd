...
```{r}
drinks=c('drink','drinks')
df_ingredient <- epicurious_recipes %>% 
  select(-one_of(state$lemma, meal, season, holiday, technique, drinks)) %>%
  na.omit()
```
...
```{r}
data = df_ingredient%>% 
  select(rating_bkt,alcoholic:zucchini)%>%
       mutate_all(funs(factor(.))) 
```

```{r}
data = epicurious_recipes %>% 
  select(3:ncol(epicurious_recipes))%>%
       mutate_all(funs(factor(.))) 
```
...
tree <- rpart(rating_bkt ~ ., data=data)
```
```{r}
prp(tree)
```
