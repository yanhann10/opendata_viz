...
df %>%
  ...
  geom_bar(stat='identity') +
  scale_fill_manual(values = pal)+
  geom_label(aes(y=0, label = ifelse(value<9, skill, ""),hjust=1), col="white", size=3, 
             label.padding = unit(0.1, "lines"), alpha=0.96) + 
  coord_polar(theta = "y") +
...
df %>%
  ...
  geom_bar(stat='identity') +
  scale_fill_manual(values = pal)+
  geom_text(aes(y = 0, label = ifelse(value > 9, skill, ""),hjust=1), size=3) +
  coord_polar(theta = "y") +
  facet_wrap(~Sport, labeller = labeller(Sport = label_wrap_gen(18))) +
  theme_void() + my_theme() +
  theme(text = element_text(size = 12,face="bold")) +
