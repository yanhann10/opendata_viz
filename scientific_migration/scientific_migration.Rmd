...
#replace long country names with short ones
...
To avoid cluttering the chord diagram with too many countries/areas, we group non-top-10 into others
...
To understand the inbound vs outbound, I calculated these 3 metrics: 

* outbound/inbound ratio
* % of all researchers that moved out of a country and 
* % of all researchers that moved in.
...
s <- od %>%
  arrange(desc(ob_ib_ratio)) %>% head(10)
...
s <- od %>%
  arrange(desc(ob_ib_ratio)) %>% tail(10)
