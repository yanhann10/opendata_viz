...
df_phrase <- phraseHavana %>%
  map_dfr(cnt) %>% 
  cbind(phraseHavana) %>%
  select(word = phraseHavana, n) %>%
  mutate(dummy = paste0("dummy",1:length(phraseHavana)))
...
use 1 word to replace the phrase then dup it back
```{r}
df_word <- df %>% 
  mutate(txt = patternreplace(df$txt, phraseHavana, replacement))%>%
  unnest_tokens(word, txt) %>% 
  mutate(id=1:n())

df_word_first_occurrence <- df_word %>% 
  arrange((id)) %>%
  group_by(word) %>% slice(1) %>% ungroup()

df_combined <- df_word %>%
  group_by(word) %>%
  summarize(n = n()) %>%
  left_join(df_word_first_occurrence[c('word','id')], by = "word") %>%
  mutate(word=replace(word, word=="dummy1","my heart is in havana"))%>%
  mutate(word=replace(word, word=="dummy2","ooh.na-na")) %>%
  mutate(word=replace(word, word=="dummy3","took me back to east atlanta")) %>%
  arrange(id) %>%
  mutate(id = 1:n())
```
